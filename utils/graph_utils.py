...
def generate_all_dags( data : pd.DataFrame, my_score : str, with_aug_priors = False ):
    N = data.shape[1]
    ...
    for r in range(len(all_possible_edges)+1):
        for subset in itertools.combinations(all_possible_edges, r):
            ...
            if nx.is_directed_acyclic_graph(G):
                ...
                # converts the graph into a string representation of
                ...
                base_dag_dict[G_str]["DAG"] = G # store the DAG in networkx format
                base_dag_dict[G_str]["DAG_indx"] = graph_indx # store the DAG index
                base_dag_dict[G_str]['Freq'] = 1 # the true distrib generates 1 graph only
                
                # compute the total number of orderings compatible with the DAG
                base_dag_dict[G_str]['num_orderings'] = len(all_valid_orderings(G)) 
                # keep track of the number of edges in the DAG
                base_dag_dict[G_str]['num_edges'] = len(G.edges())
                
                # todo: later, we need to pass the score object as an argument instead of a string
                try:
                    if my_score == "BGe Score":
                        my_score_object = BGEscore(data=data, graph=G)
                    
                    if my_score == "Marginal Log Likelihood":
                        my_score_object = MarginalLogLikelihood(data=data, graph=G)
    ...
    min_score_ordering = min([ base_dag_dict[id]["log_score_ordering"] for id in base_dag_dict.keys() ])
...
def convert_graph_to_str(g: nx.DiGraph):
